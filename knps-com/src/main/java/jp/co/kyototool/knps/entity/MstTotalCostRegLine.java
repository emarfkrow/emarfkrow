...
/**
 * MST_TOTAL_COST_REG_LINE
 *
 * @author emarfkrow
 */
public class MstTotalCostRegLine implements IEntity {
     ...
     * @return MST_TOTAL_COST_REG_LINE
     */
    public static MstTotalCostRegLine get(final Object param1, final Object param2, final Object param3) {
        ...
        String sql = "SELECT * FROM MST_TOTAL_COST_REG_LINE WHERE " + String.join(" AND ", whereList);
        ...
        return Queries.get(sql, params, MstTotalCostRegLine.class);
    }

    /**
     * MST_TOTAL_COST_REG_LINE追加
     *
     * @param now システム日時
     * @param id 登録者
     * @return 追加件数
    ...
    public int insert(final LocalDateTime now, final String id) {
        ...
        if (this.mfgTotalCostRegLineYs != null) {
            for (MfgTotalCostRegLineY mfgTotalCostRegLineY : this.mfgTotalCostRegLineYs) {
                mfgTotalCostRegLineY.setHinban(this.getHinban());
                mfgTotalCostRegLineY.setProGroupNo(this.getProGroupNo());
                mfgTotalCostRegLineY.setRouting(this.getRouting());
                mfgTotalCostRegLineY.insert(now, id);
            }
        }

        // MST_TOTAL_COST_REG_LINE_Qの登録
        if (this.mstTotalCostRegLineQ != null) {
            this.mstTotalCostRegLineQ.setHinban(this.getHinban());
            this.mstTotalCostRegLineQ.setProGroupNo(this.getProGroupNo());
            this.mstTotalCostRegLineQ.setRouting(this.getRouting());
            this.mstTotalCostRegLineQ.insert(now, id);
        }

        // MST_TOTAL_COST_REG_LINEの登録
        ...
        String sql = "INSERT INTO MST_TOTAL_COST_REG_LINE(\r\n      " + name + "\r\n) VALUES (\r\n      " + getValues() + "\r\n)";
    ...
    public int update(final LocalDateTime now, final String id) {
        ...
        if (this.mfgTotalCostRegLineYs != null) {
            for (MfgTotalCostRegLineY mfgTotalCostRegLineY : this.mfgTotalCostRegLineYs) {
                mfgTotalCostRegLineY.setHinban(this.hinban);
                mfgTotalCostRegLineY.setProGroupNo(this.proGroupNo);
                mfgTotalCostRegLineY.setRouting(this.routing);
                try {
                    mfgTotalCostRegLineY.insert(now, id);
                } catch (Exception e) {
                    mfgTotalCostRegLineY.update(now, id);
                }
            }
            this.mfgTotalCostRegLineYs = null;
            this.referMfgTotalCostRegLineYs();
            if (this.mfgTotalCostRegLineYs != null) {
                for (MfgTotalCostRegLineY mfgTotalCostRegLineY : this.mfgTotalCostRegLineYs) {
                    if (!mfgTotalCostRegLineY.getTimeStampChange().equals(now)) {
                        mfgTotalCostRegLineY.delete();
                    }
                }
            }
        }
        ...
        // MST_TOTAL_COST_REG_LINE_Qの登録
        if (this.mstTotalCostRegLineQ != null) {
            mstTotalCostRegLineQ.setHinban(this.getHinban());
            mstTotalCostRegLineQ.setProGroupNo(this.getProGroupNo());
            mstTotalCostRegLineQ.setRouting(this.getRouting());
            try {
                mstTotalCostRegLineQ.insert(now, id);
            } catch (Exception e) {
                mstTotalCostRegLineQ.update(now, id);
            }
        }

        // MST_TOTAL_COST_REG_LINEの登録
        String sql = "UPDATE MST_TOTAL_COST_REG_LINE\r\nSET\r\n      " + getSet() + "\r\nWHERE\r\n    " + getWhere();
        Map<String, Object> params = toMap(now, id);
        return Queries.regist(sql, params);
    }
    ...
    public int delete() {
        ...
        // MST_TOTAL_COST_REG_LINE_Qの削除
        if (this.mstTotalCostRegLineQ != null) {
            this.mstTotalCostRegLineQ.delete();
        }

        // MST_TOTAL_COST_REG_LINEの削除
        String sql = "DELETE FROM MST_TOTAL_COST_REG_LINE WHERE " + getWhere();
    ...
    /**
     * MST_TOTAL_COST_REG_LINE_Q
     */
    private MstTotalCostRegLineQ mstTotalCostRegLineQ;

    /**
     * @return MST_TOTAL_COST_REG_LINE_Q
     */
    @com.fasterxml.jackson.annotation.JsonProperty("MstTotalCostRegLineQ")
    public MstTotalCostRegLineQ getMstTotalCostRegLineQ() {
        return this.mstTotalCostRegLineQ;
    }

    /**
     * @param p MST_TOTAL_COST_REG_LINE_Q
     */
    public void setMstTotalCostRegLineQ(final MstTotalCostRegLineQ p) {
        this.mstTotalCostRegLineQ = p;
    }

    /**
     * @return MST_TOTAL_COST_REG_LINE_Q
     */
    public MstTotalCostRegLineQ referMstTotalCostRegLineQ() {
        if (this.mstTotalCostRegLineQ == null) {
            try {
                this.mstTotalCostRegLineQ = MstTotalCostRegLineQ.get(this.hinban, this.proGroupNo, this.routing);
    ...
    /**
     * MFG_TOTAL_COST_REG_LINE_Yのリスト
     */
    private List<MfgTotalCostRegLineY> mfgTotalCostRegLineYs;

    /**
     * @return MFG_TOTAL_COST_REG_LINE_Yのリスト
     */
    @com.fasterxml.jackson.annotation.JsonProperty("MfgTotalCostRegLineYs")
    public List<MfgTotalCostRegLineY> getMfgTotalCostRegLineYs() {
        return this.mfgTotalCostRegLineYs;
    }

    /**
     * @param list MFG_TOTAL_COST_REG_LINE_Yのリスト
     */
    public void setMfgTotalCostRegLineYs(final List<MfgTotalCostRegLineY> list) {
        this.mfgTotalCostRegLineYs = list;
    }

    /**
     * @param mfgTotalCostRegLineY
     */
    public void addMfgTotalCostRegLineYs(final MfgTotalCostRegLineY mfgTotalCostRegLineY) {
        if (this.mfgTotalCostRegLineYs == null) {
            this.mfgTotalCostRegLineYs = new ArrayList<MfgTotalCostRegLineY>();
        }
        this.mfgTotalCostRegLineYs.add(mfgTotalCostRegLineY);
    }

    /**
     * @return MFG_TOTAL_COST_REG_LINE_Yのリスト
     */
    public List<MfgTotalCostRegLineY> referMfgTotalCostRegLineYs() {
        if (this.mfgTotalCostRegLineYs == null) {
            this.mfgTotalCostRegLineYs = MstTotalCostRegLine.referMfgTotalCostRegLineYs(this.hinban, this.proGroupNo, this.routing);
        }
        return this.mfgTotalCostRegLineYs;
    }
     ...
     * @param param3 routing
     * @return List<MfgTotalCostRegLineY>
     */
    public static List<MfgTotalCostRegLineY> referMfgTotalCostRegLineYs(final String param1, final java.math.BigDecimal param2, final java.math.BigDecimal param3) {
        ...
        return Queries.select(sql, params, MfgTotalCostRegLineY.class);
    }
}
