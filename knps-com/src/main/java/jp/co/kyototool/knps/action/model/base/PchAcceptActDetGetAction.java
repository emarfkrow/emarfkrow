package jp.co.kyototool.knps.action.model.base;

import java.time.LocalDateTime;
import java.util.HashMap;
import java.util.Map;

import jp.co.kyototool.knps.entity.PchAcceptActDet;

import jp.co.golorp.emarf.action.BaseAction;

/**
 * PCH_ACCEPT_ACT_DET照会
 *
 * @author emarfkrow
 */
public class PchAcceptActDetGetAction extends BaseAction {

    /** PCH_ACCEPT_ACT_DET照会処理 */
    @Override
    public Map<String, Object> running(final LocalDateTime now, final String id, final Map<String, Object> postJson) {

        Map<String, Object> map = new HashMap<String, Object>();

        // 主キーが不足していたら終了
        Object actNo = postJson.get("actNo");
        if (actNo == null) {
            actNo = postJson.get("PchAcceptActDet.actNo");
        }
        if (actNo == null) {
            return map;
        }
        Object actBranchNo = postJson.get("actBranchNo");
        if (actBranchNo == null) {
            actBranchNo = postJson.get("PchAcceptActDet.actBranchNo");
        }
        if (actBranchNo == null) {
            return map;
        }

        PchAcceptActDet pchAcceptActDet = PchAcceptActDet.get(actNo, actBranchNo);
        map.put("PchAcceptActDet", pchAcceptActDet);
        return map;
    }

}
