package jp.co.kyototool.knps.form.model.base;

import java.util.List;
import java.util.Map;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import jakarta.validation.Valid;
import jp.co.golorp.emarf.process.BaseProcess;
import jp.co.golorp.emarf.validation.IForm;

/**
 * MST_GAMEN_PERMISSION一覧登録フォーム
 *
 * @author emarfkrow
 */
public class MstGamenPermissionSRegistForm implements IForm {

    /** logger */
    private static final Logger LOG = LoggerFactory.getLogger(MstGamenPermissionRegistForm.class);

    /** MST_GAMEN_PERMISSION登録フォームのリスト */
    @Valid
    private List<MstGamenPermissionRegistForm> mstGamenPermissionGrid;

    /**
     * @return MST_GAMEN_PERMISSION登録フォームのリスト
     */
    public List<MstGamenPermissionRegistForm> getMstGamenPermissionGrid() {
        return mstGamenPermissionGrid;
    }

    /**
     * @param p MST_GAMEN_PERMISSION登録フォームのリスト
     */
    public void setMstGamenPermissionGrid(final List<MstGamenPermissionRegistForm> p) {
        this.mstGamenPermissionGrid = p;
    }

    /** 関連チェック */
    @Override
    public void validate(final Map<String, String> errors, final BaseProcess baseProcess) {
        LOG.debug("not overridden in subclasses.");
    }

}
