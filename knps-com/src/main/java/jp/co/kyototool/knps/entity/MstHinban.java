 ...
 * @author emarfkrow
 */
public class MstHinban implements IEntity {
     ...
     * @return MST_HINBAN
     */
    public static MstHinban get(final Object param1) {

        List<String> whereList = new ArrayList<String>();
        whereList.add("TRIM (\"HINBAN\") = TRIM (:hinban)");

        String sql = "SELECT * FROM MST_HINBAN WHERE " + String.join(" AND ", whereList);

        Map<String, Object> params = new HashMap<String, Object>();
        params.put("hinban", param1);

        return Queries.get(sql, params, MstHinban.class);
    }
    ...
    public int insert(final LocalDateTime now, final String id) {
        ...
        if (this.invPrintWarehousingslips != null) {
            for (InvPrintWarehousingslip invPrintWarehousingslip : this.invPrintWarehousingslips) {
                invPrintWarehousingslip.setHinban(this.getHinban());
                invPrintWarehousingslip.insert(now, id);
            }
        }

        // INV_STOCK_STOREの登録
        if (this.invStockStores != null) {
            for (InvStockStore invStockStore : this.invStockStores) {
                invStockStore.setHinban(this.getHinban());
                invStockStore.insert(now, id);
            }
        }

        // MST_HINBAN_BKの登録
        if (this.mstHinbanBks != null) {
            for (MstHinbanBk mstHinbanBk : this.mstHinbanBks) {
                mstHinbanBk.setHinban(this.getHinban());
                mstHinbanBk.insert(now, id);
            }
        }

        // MST_SUP_UNITPRICEの登録
        if (this.mstSupUnitprices != null) {
            for (MstSupUnitprice mstSupUnitprice : this.mstSupUnitprices) {
                mstSupUnitprice.setHinban(this.getHinban());
                mstSupUnitprice.insert(now, id);
            }
        }
        ...
        String sql = "INSERT INTO MST_HINBAN(\r\n      " + name + "\r\n) VALUES (\r\n      " + getValues() + "\r\n)";
    ...
    private void numbering() {
        ...
        String sql = "SELECT LPAD (CASE WHEN MAX(e.\"HINBAN\") IS NULL THEN 0 ELSE MAX(e.\"HINBAN\") * 1 END + 1, 25, '0') AS \"HINBAN\" FROM MST_HINBAN e WHERE e.\"HINBAN\" < '9999999999999999999999999'";
    ...
    public int update(final LocalDateTime now, final String id) {
        ...
        if (this.invPrintWarehousingslips != null) {
            for (InvPrintWarehousingslip invPrintWarehousingslip : this.invPrintWarehousingslips) {
                invPrintWarehousingslip.setHinban(this.hinban);
                try {
                    invPrintWarehousingslip.insert(now, id);
                } catch (Exception e) {
                    invPrintWarehousingslip.update(now, id);
                }
            }
            this.invPrintWarehousingslips = null;
            this.referInvPrintWarehousingslips();
            if (this.invPrintWarehousingslips != null) {
                for (InvPrintWarehousingslip invPrintWarehousingslip : this.invPrintWarehousingslips) {
                    if (!invPrintWarehousingslip.getTimeStampChange().equals(now)) {
                        invPrintWarehousingslip.delete();
                    }
                }
            }
        }
        ...
        if (this.invStockStores != null) {
            for (InvStockStore invStockStore : this.invStockStores) {
                invStockStore.setHinban(this.hinban);
                try {
                    invStockStore.insert(now, id);
                } catch (Exception e) {
                    invStockStore.update(now, id);
                }
            }
            this.invStockStores = null;
            this.referInvStockStores();
            if (this.invStockStores != null) {
                for (InvStockStore invStockStore : this.invStockStores) {
                    if (!invStockStore.getTimeStampChange().equals(now)) {
                        invStockStore.delete();
                    }
                }
            }
        }
        ...
        if (this.mstHinbanBks != null) {
            for (MstHinbanBk mstHinbanBk : this.mstHinbanBks) {
                mstHinbanBk.setHinban(this.hinban);
                try {
                    mstHinbanBk.insert(now, id);
                } catch (Exception e) {
                    mstHinbanBk.update(now, id);
                }
            }
            this.mstHinbanBks = null;
            this.referMstHinbanBks();
            if (this.mstHinbanBks != null) {
                for (MstHinbanBk mstHinbanBk : this.mstHinbanBks) {
                    if (!mstHinbanBk.getTimeStampChange().equals(now)) {
                        mstHinbanBk.delete();
                    }
                }
            }
        ...
        if (this.mstSupUnitprices != null) {
            for (MstSupUnitprice mstSupUnitprice : this.mstSupUnitprices) {
                mstSupUnitprice.setHinban(this.hinban);
                try {
                    mstSupUnitprice.insert(now, id);
                } catch (Exception e) {
                    mstSupUnitprice.update(now, id);
                }
            }
            this.mstSupUnitprices = null;
            this.referMstSupUnitprices();
            if (this.mstSupUnitprices != null) {
                for (MstSupUnitprice mstSupUnitprice : this.mstSupUnitprices) {
                    if (!mstSupUnitprice.getTimeStampChange().equals(now)) {
                        mstSupUnitprice.delete();
                    }
                }
            }
        }
    ...
    public int delete() {
        ...
        // MST_HINBANの削除
        String sql = "DELETE FROM MST_HINBAN WHERE " + getWhere();
    ...
    /**
     * INV_PRINT_WAREHOUSINGSLIPのリスト
     */
    private List<InvPrintWarehousingslip> invPrintWarehousingslips;

    /**
     * @return INV_PRINT_WAREHOUSINGSLIPのリスト
     */
    @com.fasterxml.jackson.annotation.JsonProperty("InvPrintWarehousingslips")
    public List<InvPrintWarehousingslip> getInvPrintWarehousingslips() {
        return this.invPrintWarehousingslips;
    }

    /**
     * @param list INV_PRINT_WAREHOUSINGSLIPのリスト
     */
    public void setInvPrintWarehousingslips(final List<InvPrintWarehousingslip> list) {
        this.invPrintWarehousingslips = list;
    }

    /**
     * @param invPrintWarehousingslip
     */
    public void addInvPrintWarehousingslips(final InvPrintWarehousingslip invPrintWarehousingslip) {
        if (this.invPrintWarehousingslips == null) {
            this.invPrintWarehousingslips = new ArrayList<InvPrintWarehousingslip>();
        }
        this.invPrintWarehousingslips.add(invPrintWarehousingslip);
    }

    /**
     * @return INV_PRINT_WAREHOUSINGSLIPのリスト
     */
    public List<InvPrintWarehousingslip> referInvPrintWarehousingslips() {
        if (this.invPrintWarehousingslips == null) {
            this.invPrintWarehousingslips = MstHinban.referInvPrintWarehousingslips(this.hinban);
        }
        return this.invPrintWarehousingslips;
    }
    ...
    /**
     * @param param1 hinban
     * @return List<InvPrintWarehousingslip>
     */
    public static List<InvPrintWarehousingslip> referInvPrintWarehousingslips(final String param1) {
        ...
        return Queries.select(sql, params, InvPrintWarehousingslip.class);
    }

    /**
     * INV_STOCK_STOREのリスト
     */
    private List<InvStockStore> invStockStores;

    /**
     * @return INV_STOCK_STOREのリスト
     */
    @com.fasterxml.jackson.annotation.JsonProperty("InvStockStores")
    public List<InvStockStore> getInvStockStores() {
        return this.invStockStores;
    }

    /**
     * @param list INV_STOCK_STOREのリスト
     */
    public void setInvStockStores(final List<InvStockStore> list) {
        this.invStockStores = list;
    }

    /**
     * @param invStockStore
     */
    public void addInvStockStores(final InvStockStore invStockStore) {
        if (this.invStockStores == null) {
            this.invStockStores = new ArrayList<InvStockStore>();
        }
        this.invStockStores.add(invStockStore);
    }

    /**
     * @return INV_STOCK_STOREのリスト
     */
    public List<InvStockStore> referInvStockStores() {
        if (this.invStockStores == null) {
            this.invStockStores = MstHinban.referInvStockStores(this.hinban);
        }
        return this.invStockStores;
    }
    ...
    /**
     * @param param1 hinban
     * @return List<InvStockStore>
     */
    public static List<InvStockStore> referInvStockStores(final String param1) {
        ...
        return Queries.select(sql, params, InvStockStore.class);
    }

    /**
     * MST_HINBAN_BKのリスト
     */
    private List<MstHinbanBk> mstHinbanBks;

    /**
     * @return MST_HINBAN_BKのリスト
     */
    @com.fasterxml.jackson.annotation.JsonProperty("MstHinbanBks")
    public List<MstHinbanBk> getMstHinbanBks() {
        return this.mstHinbanBks;
    }

    /**
     * @param list MST_HINBAN_BKのリスト
     */
    public void setMstHinbanBks(final List<MstHinbanBk> list) {
        this.mstHinbanBks = list;
    }

    /**
     * @param mstHinbanBk
     */
    public void addMstHinbanBks(final MstHinbanBk mstHinbanBk) {
        if (this.mstHinbanBks == null) {
            this.mstHinbanBks = new ArrayList<MstHinbanBk>();
        }
        this.mstHinbanBks.add(mstHinbanBk);
    }

    /**
     * @return MST_HINBAN_BKのリスト
     */
    public List<MstHinbanBk> referMstHinbanBks() {
        if (this.mstHinbanBks == null) {
            this.mstHinbanBks = MstHinban.referMstHinbanBks(this.hinban);
        }
        return this.mstHinbanBks;
    }
    ...
    /**
     * @param param1 hinban
     * @return List<MstHinbanBk>
     */
    public static List<MstHinbanBk> referMstHinbanBks(final String param1) {
        ...
        return Queries.select(sql, params, MstHinbanBk.class);
    }

    /**
     * MST_SUP_UNITPRICEのリスト
     */
    private List<MstSupUnitprice> mstSupUnitprices;

    /**
     * @return MST_SUP_UNITPRICEのリスト
     */
    @com.fasterxml.jackson.annotation.JsonProperty("MstSupUnitprices")
    public List<MstSupUnitprice> getMstSupUnitprices() {
        return this.mstSupUnitprices;
    }

    /**
     * @param list MST_SUP_UNITPRICEのリスト
     */
    public void setMstSupUnitprices(final List<MstSupUnitprice> list) {
        this.mstSupUnitprices = list;
    }

    /**
     * @param mstSupUnitprice
     */
    public void addMstSupUnitprices(final MstSupUnitprice mstSupUnitprice) {
        if (this.mstSupUnitprices == null) {
            this.mstSupUnitprices = new ArrayList<MstSupUnitprice>();
        }
        this.mstSupUnitprices.add(mstSupUnitprice);
    }

    /**
     * @return MST_SUP_UNITPRICEのリスト
     */
    public List<MstSupUnitprice> referMstSupUnitprices() {
        if (this.mstSupUnitprices == null) {
            this.mstSupUnitprices = MstHinban.referMstSupUnitprices(this.hinban);
        }
        return this.mstSupUnitprices;
    }
    ...
    /**
     * @param param1 hinban
     * @return List<MstSupUnitprice>
     */
    public static List<MstSupUnitprice> referMstSupUnitprices(final String param1) {
        ...
        return Queries.select(sql, params, MstSupUnitprice.class);
    }
}
