package jp.co.kyototool.knps.form.model.base;

import java.util.List;
import java.util.Map;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import jakarta.validation.Valid;
import jp.co.golorp.emarf.process.BaseProcess;
import jp.co.golorp.emarf.validation.IForm;

/**
 * PCH_ACCEPT_ACT_DET一覧登録フォーム
 *
 * @author emarfkrow
 */
public class PchAcceptActDetSRegistForm implements IForm {

    /** logger */
    private static final Logger LOG = LoggerFactory.getLogger(PchAcceptActDetRegistForm.class);

    /** PCH_ACCEPT_ACT_DET登録フォームのリスト */
    @Valid
    private List<PchAcceptActDetRegistForm> pchAcceptActDetGrid;

    /**
     * @return PCH_ACCEPT_ACT_DET登録フォームのリスト
     */
    public List<PchAcceptActDetRegistForm> getPchAcceptActDetGrid() {
        return pchAcceptActDetGrid;
    }

    /**
     * @param p PCH_ACCEPT_ACT_DET登録フォームのリスト
     */
    public void setPchAcceptActDetGrid(final List<PchAcceptActDetRegistForm> p) {
        this.pchAcceptActDetGrid = p;
    }

    /** 関連チェック */
    @Override
    public void validate(final Map<String, String> errors, final BaseProcess baseProcess) {
        LOG.debug("not overridden in subclasses.");
    }

}
