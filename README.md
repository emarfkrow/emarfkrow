...
働き手不足が進行する中、業務システム開発の現場では、  
**限られた人員で、より多くのシステムを安定した品質で開発・保守すること**  
が求められています。

本OSSは、既存のデータベース定義（DDL）を起点に、  
検索画面・CRUD単票画面を含む業務Webアプリケーションを**自動生成**します。
...
というアプローチにより、**人に依存しすぎない業務Webシステム開発**を実現します。

### COBOL資産を蓄積してきた中小企業の解決策として

本OSSの主目的は、いわゆる「モダン化」や技術スタックの刷新そのものではありません。

COBOL時代から蓄積されてきたデータ資産について、  
**その意味・責任範囲・前提条件をデータモデルとして整理し、将来にわたって保守・拡張が可能な状態に再設計**  
することを選択しても、システムのモダナイズが大きな負担にならないことを、副産物として考えています。

技術的なモダナイズは、その結果として自然に実現されるものであるべきだと考えています。  

そのため、
- 業務の意味が整理されていない状態での全面刷新
- 自動変換のみを前提とした短期移行
- 現場の業務理解を伴わない設計変更

は、本OSSの想定する利用方法ではありません。
...
| [DataSources.properties](../../../sample/blob/main/samplecom/src/main/resources)   | データソース定義 | 生成時 |
