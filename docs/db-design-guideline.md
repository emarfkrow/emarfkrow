...
本章では、**必須（NOT NULL）外部キー** を用いたテーブル間の関連について、  
外部キーの参照数および依存関係の構造に基づく  
モデル関係パターンを定義する。

---

### 1. 派生パターン（Derived Pattern）
...
- **派生モデル**  
  同一の必須外部キーを持つテーブルが複数存在する場合、  
  それらのテーブルを派生モデルとする。

- **起源モデル**  
  当該テーブルの主キーを、  
  必須項目として参照するテーブルが複数存在するテーブル。

---

### 2. 転生パターン（Reincarnation Pattern）
...
- **転生モデル**  
  外部キーが必須項目であり、  
  かつ参照先テーブルが一つのみであるテーブル。

- **前世モデル**  
  当該テーブルの主キーを、  
  必須項目として参照するテーブルが一つのみ存在するテーブル。

---

### 3. 共生パターン（Symbiotic Pattern）
